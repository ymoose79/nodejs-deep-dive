            ...
            checks.forEach(function (check) {
                ...
                _data.read('checks', check, function (err, originalCheckData) {
                    if (!err && originalCheckData) {
                        ...
                        debug('err reading one of the checks data')
                    }
                })
            })
        } else {
...
workers.validateCheckData = function (originalCheckData) {
    ...
    if (id &&
        ...
        debug('err: one of the checks is not properly formated')
    }
}
...
workers.alertUserToStatusChange = function (newCheckData) {
    ...
    helpers.sendTwilioSms(newCheckData.userPhone, msg, function (err) {
        if (!err) {
            ...
            debug('err:  could not send alert to user who had a state change in their check')
        }
    })
}
                    ...
                    if (!err) {
                        ...
                        _logs.truncate(logId, function (err) {
                            if (!err) {
                                ...
                                debug('error truncating')
                            }
                        })
                    } else {
                        debug('err compressing one of the log files', err)
                    }
                })
            })
        } else {
            debug('err:  coould not find any logs')
        }
...
workers.init = function () {
    ...
    console.log('\x1b[33m%s\x1b[0m', 'Background workders are running')
